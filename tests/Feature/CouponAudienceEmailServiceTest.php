...
class CouponAudienceEmailServiceTest extends TestCase
{
    protected function setUp(): void
    {
        ...
        Schema::create('stj_cupones_header', function (Blueprint $t) { $t->id('che_id'); foreach (['che_para','che_estado','che_nombre','che_nombre_comercial','che_tipo','che_aplica','che_checkout','che_aplica_promo','che_aplica_monto_minimo','che_solo_primera_compra','che_tipo_productos','che_multiple'] as $c) $t->string($c)->nullable(); $t->decimal('che_descuento')->nullable(); $t->decimal('che_monto')->nullable(); $t->decimal('che_monto_minimo')->nullable(); $t->dateTime('che_inicio')->nullable(); $t->dateTime('che_final')->nullable(); $t->unsignedBigInteger('che_pais'); $t->unsignedBigInteger('che_genero')->nullable(); $t->unsignedBigInteger('che_coleccion')->nullable(); });
