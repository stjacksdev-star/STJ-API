...
class StorefrontRecommendationService
{
    public function recommend(string $countryCode, string $placement, StorefrontVisitor $visitor, ?StorefrontCustomer $customer = null, ?int $productId = null, int $limit = 10): array
    {
        ...
        $ranked = $candidates->map(function ($candidate) use ($seeds, $popular) {
            ...
            foreach ($seeds as $seed) {
                $score = 0; $reason = 'POPULAR';
                if ($this->same($candidate->pro_coleccion, $seed->pro_coleccion)) { $score += 40; $reason = 'SAME_COLLECTION'; }
                if ((int) $candidate->pro_categoria === (int) $seed->pro_categoria) { $score += 25; if ($reason === 'POPULAR') $reason = 'SAME_CATEGORY'; }
                if ($this->same($candidate->pro_marca, $seed->pro_marca)) { $score += 15; if ($reason === 'POPULAR') $reason = 'SAME_BRAND'; }
                if ($this->same($candidate->pro_oc_genero, $seed->pro_oc_genero)) $score += 10;
                $distance = abs((float) $candidate->ppa_precio - (float) ($seed->ppa_precio ?? $candidate->ppa_precio));
                $score += max(0, 10 - min(10, $distance));
                if ($reason === 'POPULAR' && $distance <= 5) $reason = 'SIMILAR_PRICE';
                $score += min(10, (int) ($popular[$candidate->pro_id] ?? 0));
                if ($score > $best['score']) $best = compact('score', 'reason');
            }
    ...
    private function recentlyViewed(int $countryId, StorefrontVisitor $visitor, ?StorefrontCustomer $customer, ?StorefrontCart $cart, int $limit): array
    {
        ...
        return $this->available($rows, $cart, $countryId, strtolower($code))->take($limit)->values()->map(function ($row, $i) use ($code) { $row->recommendation_reason = 'RECENTLY_VIEWED'; return $this->normalize($row, $i + 1, $code); })->all();
    }
    ...
    private function available(Collection $rows, ?StorefrontCart $cart, int $countryId, string $countryCode): Collection
    {
        ...
        $skus = DB::table('stj_inventario')->where('inv_pais', $countryId)->where('inv_tienda', $store)->where('inv_cantidad', '>', 0)->whereIn('inv_codigo', $rows->pluck('pro_codigo'))->pluck('inv_codigo')->all();
        return $rows->whereIn('pro_codigo', $skus);
    }
