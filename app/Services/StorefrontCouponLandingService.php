...
class StorefrontCouponLandingService
{
    ...
    public function find(string $countryCode, int $headerId): ?array
    {
        ...
        $rowsQuery = DB::table('stj_cupones_producto as cp')->join('stj_productos as product', 'product.pro_id', '=', 'cp.cpr_producto')
            ->join('stj_producto_pais as country_product', function ($join) use ($header) {
                $join->on('country_product.ppa_producto', '=', 'product.pro_id')->where('country_product.ppa_pais', (int) $header->pai_id);
            })
        ...
        $rows = $rowsQuery
            ->orderBy('cp.cpr_id')->get(['product.pro_id', 'product.pro_codigo', 'product.pro_nombre', 'product.pro_descripcion', 'product.pro_marca', 'product.pro_thumbs', 'product.pro_tallas', 'country_product.ppa_precio', 'country_product.ppa_es_popular', 'category.cat_nombre as categoria_nombre', 'subcategory.sca_nombre as subcategoria_nombre']);
        ...
        $products = $rows->map(function ($product) use ($header, $stock) {
            ...
            $available = $stock['availabilityBySku'][$sku] ?? null;
            $category = trim((string) ($product->categoria_nombre ?: 'Cupón'));
            ...
            return ['id' => (int) $product->pro_id, 'name' => trim((string) $product->pro_nombre), 'slug' => Str::slug((string) $product->pro_nombre).'-'.$product->pro_id, 'sku' => $sku,
                'price' => (float) $product->ppa_precio, 'currency' => $this->currency(strtolower($header->pai_codigo)), 'badge' => $product->ppa_es_popular ? 'Popular' : 'Disponible',
        ...
        return ['coupon' => ['headerId' => (int) $header->che_id, 'name' => trim((string) ($header->che_nombre_comercial ?: $header->che_nombre)), 'slug' => Str::slug((string) ($header->che_nombre_comercial ?: $header->che_nombre)),
            'benefit' => match ($header->che_tipo) {
                'DESCUENTO' => $this->number($header->che_descuento).' % de descuento', 'PRECIO' => 'Precio especial de '.$this->currency(strtolower($header->pai_codigo)).' '.$this->number($header->che_monto), default => 'Envío gratis'
            },
    ...
    private function currency(string $country): string
    {
        return match ($country) {
            'gt' => 'GTQ', 'cr' => 'CRC', 'hn' => 'HNL', default => 'USD'
        };
    }
