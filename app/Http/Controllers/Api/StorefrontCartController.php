...
class StorefrontCartController extends BaseController
{
    ...
    public function updateItem(Request $request,string $country,int $item): JsonResponse
    {
        ...
        if(!array_key_exists('quantity',$data)&&!array_key_exists('selected',$data)) return $this->error('Debe indicar quantity o selected.',422);
    ...
    {
        $data=$request->validate(['operation_uuid'=>['required','uuid']]); $customer=$this->customer();
        if(!$customer)return $this->error('La combinacion requiere un cliente autenticado.',401);
        return $this->mutation(fn()=>$this->carts->merge($country,$this->visitor($request),$customer,$data),'Carritos combinados.');
    }

    private function itemRules(): array {return ['operation_uuid'=>['required','uuid'],'product_id'=>['required','integer'],'sku'=>['required','string','max:50'],'size'=>['required','string','max:10'],'quantity'=>['required','integer','min:1','max:99']];}
    private function visitor(Request $request): StorefrontVisitor {return $request->attributes->get('storefrontVisitor');}
    private function customer(): ?StorefrontCustomer {$user=Auth::guard('sanctum')->user();return $user instanceof StorefrontCustomer?$user:null;}
    private function mutation(callable $callback,string $message): JsonResponse {try{return $this->success($callback(),$message);}catch(CartOperationConflict $e){return $this->error($e->getMessage(),409);}}
}
